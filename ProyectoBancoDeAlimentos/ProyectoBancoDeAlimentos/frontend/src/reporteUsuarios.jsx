...
const ReporteUsuarios = () => {
  ...
  const usuarios = [
    ...
    { id: '002', usuario: 'juanperez', fecha: '14/08/2024', estado: 'Inactivo' },
    { id: '003', usuario: 'anagarcia', fecha: '13/08/2024', estado: 'Activo' },
    { id: '004', usuario: 'carloslopez', fecha: '12/08/2024', estado: 'Activo' },
  ...
  const usuariosFiltrados = useMemo(() => {
    ...
    return usuarios.filter(user => 
      filtroEstado === 'activos' ? user.estado === 'Activo' : user.estado === 'Inactivo'
    );
  }, [filtroEstado]);
        ...
        <div className="tabla-usuarios">
          ...
          {usuariosFiltrados.map((user, index) => (
            <div key={index} className="usuario-row">
              ...
              <span className={`estado-badge ${user.estado.toLowerCase() === 'activo' ? 'estado-activo' : 'estado-inactivo'}`}>
                {user.estado}
              </span>
          ...
          <div className="stats-card">
            ...
            <div className="leyendas">
              <div className="leyenda-item">
                ...
                <span className="leyenda-porcentaje">{estadisticas.inactivos}%</span>
              </div>
              <div className="leyenda-item">
                <div className="leyenda-color leyenda-activos"></div>
                <span className="leyenda-texto">Activos</span>
                <span className="leyenda-porcentaje">{estadisticas.activos}%</span>
              </div>
            </div>
          </div>
            ...
            <div className="grafico-barras-clientes">
              {Object.entries(datosMensuales).map(([mes, cantidad]) => (
                <div key={mes} className="barra-mes">
                  ...
                  ></div>
                  <span className="barra-label">{mes.charAt(0).toUpperCase() + mes.slice(1)}</span>
                </div>
              ))}
            </div>
